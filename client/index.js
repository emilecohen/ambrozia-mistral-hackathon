...
const heygen_API = {
  apiKey: 'ZjU0MmNmNzBlNGEzNDA2MjgxODcyYjRkMTZkMWJiOWQtMTcwODk5NzEzMA==',
...
async function talkToOpenAI(prompt) {
  ...
  const response = await fetch(`${SERVER_URL}/chat/complete`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
async function generateCompanyKnowledge(url) {
  console.log(JSON.stringify({ url }));
  const response = await fetch(`${SERVER_URL}/generate-knowledge`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Access-Control-Allow-Origin': '*',
  ...
  if (response.status === 500) {
    ...
    console.log('data: ', data.message)
    localStorage.setItem('companyKnowledge', data.message);
